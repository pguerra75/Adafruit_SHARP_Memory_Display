...
byte sharpmem_buffer[((SHARPMEM_LCDWIDTH / 8) * SHARPMEM_LCDHEIGHT)];
...
void Adafruit_SharpMem::drawPixel(int16_t x, int16_t y, uint16_t color) 
{
  ...
  if(color) {
    sharpmem_buffer[((long)y*SHARPMEM_LCDWIDTH + x) / 8] |=
...
uint8_t Adafruit_SharpMem::getPixel(uint16_t x, uint16_t y)
{
  ...
  return sharpmem_buffer[((long)y*SHARPMEM_LCDWIDTH + x) / 8] &
...
{
  memset(sharpmem_buffer, 0xff, ((SHARPMEM_LCDWIDTH / 8) * SHARPMEM_LCDHEIGHT));
...
void Adafruit_SharpMem::refresh(void) 
{
  ...
  totalbytes = ((SHARPMEM_LCDWIDTH / 8) * SHARPMEM_LCDHEIGHT);
...
//Rather then clear the screen between write, clear the buffer and simply write over the current screen. This should save time.
void Adafruit_SharpMem::clearBuffer(void)
{
  memset(sharpmem_buffer, B11111111, 12000);
}